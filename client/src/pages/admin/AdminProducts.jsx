...
import { Package, Plus, AlertCircle, Image as ImageIcon, Type, CheckCircle, X, Palette, Ruler, PlusCircle, Trash2 } from 'lucide-react';
...
export default function AdminProducts() {
    ...
    const addColor = (hex) => {
        if (!colors.find(c => c.value === hex)) {
            setColors([...colors, { value: hex, price: '', imageBase64: '' }]);
        }
    };
    ...
    const handleCreateProduct = async (e) => {
        ...
        try {
            ...
            setSuccess(`Product "${prodName}" added successfully!`);
                    ...
                    <CardContent className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" required value={prodName} onChange={(e) => setProdName(e.target.value)} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder="e.g. Magic Mug" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Base Price (LKR)</label>
                                <input type="number" required min="0" value={prodPrice} onChange={(e) => setProdPrice(e.target.value)} className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500" placeholder="1500" />
                            </div>
                            <div>
                                ...
                                {colors.map((color, idx) => (
                                    <div key={idx} className="flex flex-wrap md:flex-nowrap items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                                        ...
                                        <div className="flex items-center gap-2 bg-white px-2 py-1 rounded-lg border border-gray-200 shrink-0">
                                            {color.imageBase64 && <img src={color.imageBase64} alt="var" className="h-6 w-6 rounded object-cover" />}
                            ...
                            <div className="space-y-3">
                                {sizes.map((size, idx) => (
                                    <div key={idx} className="flex flex-wrap md:flex-nowrap items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                                        ...
                                        <div className="flex items-center gap-2 bg-white px-2 py-1 rounded-lg border border-gray-200 shrink-0">
                                            {size.imageBase64 && <img src={size.imageBase64} alt="var" className="h-6 w-6 rounded object-cover" />}
                                        ...
                                        <div className="space-y-3 pl-4 border-l-2 border-indigo-200">
                                            {cv.options.map((opt, optIdx) => (
                                                <div key={optIdx} className="flex flex-wrap md:flex-nowrap items-center gap-3">
                                                    ...
                                                    <div className="flex items-center gap-2 bg-white px-2 py-1 rounded-lg border border-gray-200 shrink-0">
                                                        {opt.imageBase64 && <img src={opt.imageBase64} alt="var" className="h-6 w-6 rounded object-cover" />}
